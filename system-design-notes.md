...
* Horizontal scaling: Adding more servers to a system will lead to a proportional increase in performance.  Not everything can be scaled like this without a redesign, but it has a very high ceiling
* Vertical scaling: Simpler than horizontal but has a lower ceiling. Although, some modern specialised servers are exceptionally powerful (eg. a 2Tb Redis Cache is available on AWS)  
...
* UDP/TCP: TCP provides reliable data transmission even when the data is split into packets, some of which may be routed differently, and reassembles the data as it was sent by the sender. Used for interacting with systems that prioritise reliability over performance. UDP on the other hand is fire-and-forget, a % of packet loss is acceptable and strict ordering isn’t a problem. UDP would often be used for video streaming and other data streaming.
